import { useState, useEffect } from 'react'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Card, CardHeader, CardTitle, CardContent } from '@/components/ui/card'
import { listSubmissions, createArchive, getManifest, type Submission, type ArchiveConfig } from '@/lib/api'
import { Loader2, Package, FileText } from 'lucide-react'

export function ArchivePanel() {
  const [submissions, setSubmissions] = useState<Submission[]>([])
  const [selected, setSelected] = useState<number[]>([])
  const [loading, setLoading] = useState(true)
  const [archiving, setArchiving] = useState(false)
  const [manifest, setManifest] = useState<string | null>(null)
  const [config, setConfig] = useState<Omit<ArchiveConfig, 'submission_ids'>>({
    naming_template: '{序号}_{姓名}_周小结_{日期范围}',
    start_number: 1,
    number_padding: 2,
  })

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await listSubmissions({ status: 'checked' })
        setSubmissions(res.data)
      } finally {
        setLoading(false)
      }
    }
    fetchData()
  }, [])

  const toggleSelect = (id: number) => {
    setSelected(prev => 
      prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]
    )
    setManifest(null)
  }

  const selectAll = () => {
    if (selected.length === submissions.length) {
      setSelected([])
    } else {
      setSelected(submissions.map(s => s.id))
    }
    setManifest(null)
  }

  const handlePreview = async () => {
    if (selected.length === 0) return
    setArchiving(true)
    try {
      const res = await getManifest({ ...config, submission_ids: selected })
      setManifest(res.data.manifest_text)
    } finally {
      setArchiving(false)
    }
  }

  const handleArchive = async () => {
    if (selected.length === 0) return
    setArchiving(true)
    try {
      const res = await createArchive({ ...config, submission_ids: selected })
      const url = window.URL.createObjectURL(new Blob([res.data]))
      const link = document.createElement('a')
      link.href = url
      link.download = 'weekly_summary_archive.zip'
      link.click()
      window.URL.revokeObjectURL(url)
    } finally {
      setArchiving(false)
    }
  }

  if (loading) {
    return (
      <Card>
        <CardContent className="flex items-center justify-center py-8">
          <Loader2 className="w-6 h-6 animate-spin" />
        </CardContent>
      </Card>
    )
  }

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Package className="w-5 h-5" />
          批量归档
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-3 gap-4">
          <div>
            <label className="text-sm font-medium mb-1 block">命名模板</label>
            <Input
              value={config.naming_template}
              onChange={e => setConfig(prev => ({ ...prev, naming_template: e.target.value }))}
              placeholder="{序号}_{姓名}_周小结_{日期范围}"
            />
          </div>
          <div>
            <label className="text-sm font-medium mb-1 block">起始序号</label>
            <Input
              type="number"
              value={config.start_number}
              onChange={e => setConfig(prev => ({ ...prev, start_number: parseInt(e.target.value) || 1 }))}
            />
          </div>
          <div>
            <label className="text-sm font-medium mb-1 block">序号位数</label>
            <Input
              type="number"
              value={config.number_padding}
              onChange={e => setConfig(prev => ({ ...prev, number_padding: parseInt(e.target.value) || 2 }))}
            />
          </div>
        </div>

        <div className="border rounded-lg">
          <div className="flex items-center justify-between p-3 border-b bg-slate-50">
            <span className="text-sm font-medium">已校对的周小结（{submissions.length} 份）</span>
            <Button size="sm" variant="outline" onClick={selectAll}>
              {selected.length === submissions.length ? '取消全选' : '全选'}
            </Button>
          </div>
          <div className="max-h-64 overflow-y-auto">
            {submissions.length === 0 ? (
              <div className="text-center text-slate-500 py-8">暂无已校对的周小结</div>
            ) : (
              submissions.map(sub => (
                <label
                  key={sub.id}
                  className="flex items-center gap-3 p-3 border-b last:border-b-0 hover:bg-slate-50 cursor-pointer"
                >
                  <input
                    type="checkbox"
                    checked={selected.includes(sub.id)}
                    onChange={() => toggleSelect(sub.id)}
                    className="w-4 h-4"
                  />
                  <span>{sub.name}</span>
                  <span className="text-slate-500 text-sm">{sub.date_range}</span>
                </label>
              ))
            )}
          </div>
        </div>

        {manifest && (
          <div className="bg-slate-50 p-4 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              <FileText className="w-4 h-4" />
              <span className="font-medium">文件清单预览</span>
            </div>
            <pre className="text-sm whitespace-pre-wrap font-mono">{manifest}</pre>
          </div>
        )}

        <div className="flex gap-3">
          <Button variant="outline" onClick={handlePreview} disabled={selected.length === 0 || archiving}>
            预览清单
          </Button>
          <Button onClick={handleArchive} disabled={selected.length === 0 || archiving}>
            {archiving ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Package className="w-4 h-4 mr-2" />}
            打包下载（{selected.length} 份）
          </Button>
        </div>
      </CardContent>
    </Card>
  )
}
